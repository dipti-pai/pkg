...
func TestClient_Options(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			opts: []OptFunc{WithApiURL(gitHubEnterpriseURL), WithInstllationID(installationID), WithAppID(appID), WithPrivateKey(kp.PrivateKey)},
		},
		{
...
func TestClient_GetToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			g.Expect(err).ToNot(HaveOccurred())
			opts := []OptFunc{
				WithApiURL(srv.URL), WithInstllationID("123"), WithAppID("456"), WithPrivateKey(kp.PrivateKey),
			}
